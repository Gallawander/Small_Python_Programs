...
def draw_hangman(x, y):
    head = canvas.create_oval(
        x-20, y, x+20, y+40, width=3, state=tkinter.HIDDEN)
    torso = canvas.create_line(x, y+40, x, y+90, width=3, state=tkinter.HIDDEN)
    larm = canvas.create_line(
        x-40, y+60, x, y+60, width=3, state=tkinter.HIDDEN)
    rarm = canvas.create_line(
        x+40, y+60, x, y+60, width=3, state=tkinter.HIDDEN)
    lleg = canvas.create_line(x, y+90, x-30, y+130,
                              width=3, state=tkinter.HIDDEN)
    rleg = canvas.create_line(x, y+90, x+30, y+130,
                              width=3, state=tkinter.HIDDEN)
...
def draw_letters(letters):
    ...
    for letter in letters:
        ...
        idx = canvas.create_text(
            start+20, 85, text=letter.upper(), font="arial 30", state=tkinter.HIDDEN)
